# Sylfira

<img class="side" src="party/images/Sylfira.webp" alt="Sylfira">

*Elf (Ancient Elf (Rouge Dedication)) Sorcerer 4*

Background: Fireworks Performer · Languages: Draconic, Elder-thing

### **Current Gold:** 96.9 gp

## Stats

| STR | DEX | CON | INT | WIS | CHA |
|-----|-----|-----|-----|-----|-----|
| +0 (10) | +3 (16) | +0 (10) | +2 (14) | +0 (10) | +4 (18) |

**Max HP** 34 · **AC** 21 · **Perception** +6 (Trained)

**Fortitude** +6 (Trained) · **Reflex** +9 (Trained) · **Will** +8 (Expert)

## Skills

| Skill | Proficiency | Bonus |
|-------|-------------|-------|
| Acrobatics | Trained | +9 |
| Arcana | Trained | +8 |
| Athletics | Trained | +6 |
| Deception | Trained | +10 |
| Diplomacy | Trained | +10 |
| Fireworks Lore | Trained | +8 |
| Intimidation | Expert | +12 |
| Occultism | Trained | +8 |
| Performance | Trained | +10 |
| Society | Trained | +8 |

## Spells

### Sorcerer *(Arcane)*

- **Cantrips:** Detect Magic, Electric Arc, Elemental Counter, Frostbite, Needle Darts, Shield
- **Rank 1:** Conductive Weapon, Fear, Leaden Steps, Soothe, Thunderstrike
- **Rank 2:** Blazing Bolt, Invisibility, Mirror Image

### Arcane Focus Tradition *(Arcane, focus)*

- **Rank 1:** Flurry of Claws

## Feats & Features

- **Class feats:** Propelling Sorcery, Rogue Dedication, Arcane Evolution
- **Ancestry feats:** Ancestral Longevity
- **Skill feats:** Fascinating Performance, Intimidating Glare, Arcane Sense, Bon Mot
- **General feats:** Toughness
- **Features:** Bloodline, Bloodline Spells, Bloodline: Draconic (Arcane Dragon), Signature Spells, Sorcerer Spellcasting, Sorcerous Potency, Spell Repertoire, Surprise Attack

## Inventory

Studded Leather · Waterskin · Hand of the Mage · Masquerade Scarf · Thieves' Toolkit · Iron Key to area B5 · Sack ×3 · Healing Potion (Lesser) · Candle · Scroll of Grease (Rank 1) · Scroll of Dispel Magic (Rank 2) · Scroll of Tailwind (Rank 2) · Scroll of Sleep (Rank 1) · One-Hour Flower · Wand of Fear (Rank 1) · Sneaky Key

---

*Auto-generated from the Foundry VTT export — derived numbers skip item/status bonuses. The GM regenerates this section, so edit **below** the line and your changes will stick.*

<!-- PLAYER NOTES — everything below this line survives sheet regeneration -->

## Player Notes

*(yours to fill in — goals, grudges, loot claims, embarrassing moments…)*
