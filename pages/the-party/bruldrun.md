# Bruldrun

<img class="side" src="party/images/Bruldrun.jpg" alt="Bruldrun">

*Male Dwarf (Ancient-Blooded Dwarf) Gunslinger 4*

Background: Gunsmith · Follows Torag · Age 130 · Languages: Common, Dwarven

### **Current Gold:** 144.73 gp

## Stats

| STR | DEX | CON | INT | WIS | CHA |
|-----|-----|-----|-----|-----|-----|
| +2 (14) | +4 (18) | +2 (14) | +0 (10) | +2 (14) | -1 (8) |

**Max HP** 54 · **AC** 21 · **Perception** +10 (Expert)

**Fortitude** +10 (Expert) · **Reflex** +12 (Expert) · **Will** +10 (Expert)

## Skills

| Skill | Proficiency | Bonus |
|-------|-------------|-------|
| Acrobatics | Trained | +10 |
| Athletics | Trained | +8 |
| Crafting | Trained | +6 |
| Engineering Lore | Trained | +6 |
| Intimidation | Trained | +5 |
| Medicine | Expert | +10 |

## Weapons

- **Dwarven War Axe** — 1d8 slashing *(dwarf, sweep, two-hand-d12)*
- **Clan Pistol** — 1d6 piercing *(concussive, dwarf, fatal-d10)*

## Feats & Features

- **Class feats:** Dual-Weapon Reload, Running Reload, Sword and Pistol
- **Ancestry feats:** Dwarven Weapon Familiarity
- **Skill feats:** Battle Medicine, Continual Recovery, Quick Repair
- **General feats:** Toughness
- **Features:** Clan Dagger, Gunslinger's Way, Slinger's Precision, Stubborn, Way of the Drifter

## Inventory

Studded Leather · Healer's Toolkit · Rope · Bedroll · Flint and Steel · Grappling Hook · Striking · Backpack · Rations · Smoke Ball (Lesser) ×3 · Beacon Shot ×6 · Storm Arrow · Rounds (Dueling Pistol) ×21 · Elemental Ammunition (Lesser, Fire) · New Ammunition · Rounds (Clan Pistol) ×78

---

*Auto-generated from the Foundry VTT export — derived numbers skip item/status bonuses. The GM regenerates this section, so edit **below** the line and your changes will stick.*

<!-- PLAYER NOTES — everything below this line survives sheet regeneration -->

## Player Notes

*(yours to fill in — goals, grudges, loot claims, embarrassing moments…)*
